...
\begin{figure*}[h]
  \begin{mathpar}
    ...
    \inferrule[Tag] { 
      ...
      \Delta\ \Gamma \vdash l\#t : l*\tau  
    }

    \inferrule[Record] { 
      \bigwedge_{(l\sim\tau) \in types} 
      \ 
      \bigwedge_{(l:=t) \in fields} 
      \Delta\ \Gamma \vdash t : \tau
    } {
      \Delta\ \Gamma \vdash \omega[fields] : (\text{reduce}\ ;\ types)  
    } 

    \inferrule[Function] {
      \bigwedge_{(\tau_1 \rightarrow \tau_2) \in types} 
      \ 
      \bigwedge_{(t_1 : \tau_1 \Rightarrow t_2) \in cases } 
      \Delta\ \Gamma_1 \vdash t_1 : \tau_1
      \ \wedge\
      \Delta\ \Gamma;;\Gamma_1 \vdash t_2 : \tau_2
    } {
      \Delta\ \Gamma \vdash \lambda[cases] : (\text{reduce}\ ;\ types)  
    } 

    \inferrule[Proj] {
      \Delta\ \Gamma \vdash t : l\sim\tau 
    } {
      \Delta\ \Gamma \vdash t/l : \tau 
    } 

    \inferrule[App] { 
      \Delta\ \Gamma \vdash t_1 : \tau_1 \rightarrow \tau
      \\
    ...
    \inferrule[Let] { 
      ...
      \\ 
      \widebar{\alpha} = \{\alpha \ |\ \alpha \in \text{freevars}\ \tau_1\ \wedge\ 
        \alpha \not\in \text{dom}(\Delta)
      \}
      \\\\
      ...
      \Delta\ \Gamma \vdash (\text{let}\ x : \tau_1 := t_1 \Rightarrow t_2) : \tau 
    } 

    \inferrule[Fix] { 
      \Delta\ \Gamma \vdash t : \tau \rightarrow \tau 
    } {
      \Delta\ \Gamma \vdash \text{fix}\ t : \tau 
    } 
    ...
    \inferrule[Top] { 
      ...
      \Delta\ \Gamma \vdash t : \top 
    } 
    \end{mathpar}
  \caption{Typing semantics}
\end{figure*}
