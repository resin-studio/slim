...
\inferrule[univ-free]
  { 
    ...
    \ \wedge\
    \\\\
    \Delta;\Delta_1 \vdash \tau_1 \leq \tau_2 \rightsquigarrow M_2
    \ \wedge\
    \Omega_2 \in M_2
    \\\\
    \} = M
...
\inferrule[recur-symm]
  { 
     \Delta \vdash \tau' \equiv \tau\ \wedge \{ \cdot \} = M\ \vee\ 
     \\\\
     \Delta \vdash \tau'[\alpha/\mu\alpha.\tau] \leq \tau[\alpha/\mu\alpha.\tau] \rightsquigarrow M 
  }
  {\Delta \vdash \mu \alpha . \tau' \leq \mu \alpha . \tau	\rightsquigarrow M}

\inferrule[recur-tag]
  {\Delta \vdash  \#l\ \tau_1 \leq \tau[\alpha/\mu \alpha . \tau] \rightsquigarrow M}
...
\inferrule[corec-symm]
  { 
     \Delta \vdash \tau' \equiv \tau\ \wedge \{ \cdot \} = M\ \vee\ 
     \\\\
     \Delta \vdash \tau'[\alpha/\nu\alpha.\tau'] \leq \tau[\alpha/\nu\alpha.\tau'] \rightsquigarrow M 
  }
  { \Delta \vdash 
    \nu \alpha . \tau' 
    \leq 
...
\inferrule
  { \tau\ \&\ \text{intersect}\ T = \tau'}
  {\text{intersect}\ \{ \tau \} \cup T = \tau'} 
...
\inferrule
  { 
    ...
    \Omega_1.\tau_1 \in N_1
    \ \wedge\ 
    \Omega_2.\tau_2 \in (\text{intersect}\ Ns)
    \\\\
    \} = N'
  }
  {\text{intersect}\ \{ N_1 \} \cup Ns = N'} 
...
\inferrule[Record]
  { 
    ...
    \\
    \text{intersect}\ \{.l\ \tau_1\ |\ (.l\ t_1, \tau_1) \in fields \} = \tau'
    \\
    \Delta\ \vdash \tau' \leq \tau \rightsquigarrow M
    \\\\
    \text{intersect}\ \{N_1\ |\ 
    \\\\
    (.l\ t_1, \tau_1) \in fields
    \ \wedge\ 
    \Omega \in M
    \ \wedge\ 
    \\\\
...
\inferrule[Function]
  {
    ...
    \\
    \text{intersect}\ \{\tau_1 \rightarrow \tau_2\ |\ (\lambda\ t_1 : \tau_1 \Rightarrow t_2, \tau_2) \in cases \} = \tau'
    \\
    \Delta\ \vdash \tau' \leq \tau \rightsquigarrow M
    \\\\
    \text{intersect}\ \{N_2\ |\ 
    ...
    \ \wedge\ 
    \\\\
    \text{patvars}\ t_1\ \tau_1 = \Gamma_x
    \ \wedge\ 
    \Omega \in M
    \ \wedge\ 
    \\\\
    \Delta;\Omega\ \Gamma;\Gamma_x \vdash t_2 : \tau_2 \rightsquigarrow N_2
    \} = N
  }
  {\Delta\ \Gamma \vdash t : \tau  
    \rightsquigarrow 
...
\inferrule[App]
  { 
    ...
    \Omega_2.\tau_2' \in N_2
    \\\\
    \Delta;\Omega_1;\Omega_2 \vdash \tau_1' \leq \tau_2' \rightarrow \beta
      \rightsquigarrow M_3
    \ \wedge\ 
    \Omega_3 \in M_3
    \\\\
...
\inferrule[Let]
  { 
    ...
    \ \wedge\ 
    \\\\
    \Delta;\Omega_1\ \Gamma,x \mapsto (\forall \beta :: \beta \leq \tau_1\ .\ \beta) 
...
\inferrule[Fix]
  { 
    ...
    \{ \Omega_1;\Omega_2\ .\ \alpha \ |\ 
    \\\\
    \Delta\ \Gamma \vdash t_1 : \tau \rightarrow \tau 
    \rightsquigarrow N_1
    \ \wedge\  
    \Omega_1\ .\ \tau_1' \in N_1
    \ \wedge\  
    \\
    \Delta;\Omega_1 \vdash \tau_1' \leq \alpha \rightarrow \alpha 
    \rightsquigarrow M_2
    \ \wedge\  
    \Omega_2 \in M_2
    \\\\
...
% Observations:
% the subtyping constraint offers three interpretations of types
  % 1. a type is a set of objects
  % 2. a type is a proposition proved by a non-empty subtype
  % 3. a type is a proof satisfying a proposition 
  % e.g. Proof <: Prop
...
% find decision procedure for relational types (subtyping over induction on compound types)
  ...
  % contraction maps?
  % metric spaces?
% find decision procedure for typing recursion? 
  % add mu/nu binding as solution for circular subtyping? 
  % abstract interpretation narrowing/widening?
    % creates fixpoint of abstract values indicating termination
    % this is not needed since typing is lexical;
    % recursion is not interpreted as in abstract interpretation / flow analysis 
