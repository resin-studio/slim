...
\inferrule[Let]
  { 
    ...
    \ \wedge\ 
    \\\\
    \Delta;\Omega_1 \vdash_{simp}  \tau_1' \rightsquigarrow \tau_1''
    \ \wedge\ 
    \text{freevars}\ \tau_1'' = \widebar{\alpha}
    \ \wedge\ 
    \\\\
    % NOTE: Remy doesn't need to simply, since their universal holds constraints 
...
% correspondence to dependent types
  ...
  % P ≡ ∃ N L :: F ≤ L -> N => (N × L) 
  % 
  % P ≡ (0^@ × nil^@) | (∃ N L :: N × L ≤ P => 1^N × cons^L) 
  % concepts from dependents types may be mimicked by 
  % basic compound types and subtyping constraints 
  % sigma performs both the internal introduction and external projection 
