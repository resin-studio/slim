...
We illustrate the syntax and semantics of programs and types with the example program shown in Figure \ref{fig:program}.
...
\paragraph{Transposition.} Suppose the function \code{fromList}, which expects a list, 
...
can safely be composed, by transposing \code{($nat\_list\ \alpha$)} into the weaker pair \code{($nat$ * $list\ \alpha$)}.
Once transposed, the list type of the argument type's relation can be projected and unified with the parameter's list type. 
\begin{mathpar}
\\
  \inferrule {
