...
\subsection{Syntax}
The syntax rules are given in Fig. \ref{fig:syntax}.
The programming language includes functions, records, fix combinator, pattern matching, let binding, 
and tags for discriminating cases in pattern matching.  
...
The type of a function may be described as an intersection of implications, 
where each implication represents the type of a path.
Likewise, the type of a record may be described as an intersection of field types.
The existential type consists of multiple bound variales, a payload containing the bound variables, 
...
The subtyping rules are given in Fig. \ref{fig:subtyping}.
...
\begin{figure*}[h]
\begin{flalign*}
  &\boxed{\F{unifyContd}(\Omega, \Delta, \tau_{l}, \tau_{r}) = C}&
\end{flalign*}
\[
\begin{array}[t]{@{} l}
