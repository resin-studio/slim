...
\TODO{update tag syntax: \code{\#cons.\#cons.e:?cons.?cons.T}\ \ \code{\#nil:?nil}}
...
The tag case reconstructs the type of its payload and uses its label to construct a tag type.
The record case reconstructs the type its fields and intersects the constructed fields types together.

The function case is of particular importance to type reconstruction in an untyped setting.
...
Before returning the reconstructed implication type for the path, the case removes
...
As with the record case, the reconstructed implications are intersected together.

The projection case reconstructs the type of the record expression. 
It then calls unify to solve for the projected type by finding
a single field type that is subtyped by the record type. 

The application case also plays an important role in type reconstruction of untyped programs.
It creates a fresh type variable as a placeholder for the reconstructed type of the application result.
It reconstructs the type of the function and the argument, and then solves for the result type
by unifying the function's type with an implication from the argument type to the result type.
It adds the result type variable to the adjustment variable set when unifying to allow reconstructing 
all types for all the paths that the function might take.
...
the let-binding case reconstructs the type of the argument and checks that
all possible types subtype the annotation. It then generalizes 
type variables in the reconstructed argument type while maintaining
the constraints indicated by the subtyping environment. 

The fix case reconstructs inductive types for the parameters and bodies of its
target expression. More it inductively relates the parameter and body types to each other.
...
\begin{array}[t]{@{} l}
    ...
    \\
    \I\I\I\I \tau_{pack} \leftarrow \F{pack}(\Omega_{stale}, \Delta, \tau_a)
    \\
    \I\I\I\I \alpha \leftarrow \F{freshTV}()
    \\
    \I\I\I\I \F{infer}(\Omega, \Delta, (\Gamma;\code{$x$:[$\alpha$<:$\tau_{pack}$]$\alpha$}), e_b)
    \\
    \I\I \B{else } \emptyset 
...
the left type indicates the weaker type, and the right type indicates the stronger type.
...
Subtyping holds but there is nothing that can be unified without circular references 
so the assumed subtyping environment is returned in a singleton set. 
...
for the variable. If a solution is found the unification proceeds
...
The left-variable rule follows is similar to the right-variable rule with
the one difference being that it uses intersection to strengthen the type variable 
...
The subsequent cases decompose the types into subproblem. 
...
that strengthen the left side or weaken the right side occur before rules that
weaken the left side or strengthen the right side.

The left existential case first tries unify the constrain. It's possible that the
constraint cannot be solved, in which case unifying the existential type's constraint 
simply updates the subtyping environment with the unsolved relational constraint. 
With the updated environment it then unified the existential type's payload with the right type.
...
IF factoring fails the procedure leverages the induction principle and substitutes the right type 
...
The right existential case simply unifies left type with the existential's payload
...
The procedures unifies the universal's payload with the right type,
which may instantiate the universal's type variable,
which may then be used unify and check the universal's constraint.
...
if more information were specified, then the unsolved relational constraint
