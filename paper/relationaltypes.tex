...
\\
  \inferrule {
    ...
    \Delta \cdot \Gamma
    \vdash 
    \code{repeat} : \code{[X,N<:$nat$] X -> N -> \{L with N * L <:\ $nat\_list$\ X\}} 
  }
\\
\end{mathpar}
...
\\
  \inferrule {
    list\ \alpha = \code{indu[L] ?nil | ?cons ($\alpha$ * L)} 
    \\\\
...
\\
  \inferrule {
    \inferrule {
      even = \code{indu[N] ?zero | ?succ ?succ N} 
      \\\\
...
\begin{mathpar}
  \inferrule { 
    \F{freeTVs}(\Delta) \cap \widebar{\alpha} \subseteq \emptyset 
    \\\\
    \bigand_{\Delta'}
  ...
  \inferrule { 
    ...
    \subtypes \code{indu[$\alpha_2$]$\tau_2$}
  }
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
%% Subtype unification: basic rules 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
\begin{figure*}[h]
\begin{flalign*}
...
\begin{array}[t]{@{} l}
    ...
    \\
    %% induction 
    \I \B{case } \code{indu[$\alpha_1$]$\tau_1$}, \code{indu[$\alpha_2$]$\tau_2$} :
    \\
    ...
    %% left-induction
    \I \B{case } \code{indu[$\alpha$]$\tau$}, \tau_r :
    \\
    \I\I \tau_{fac} \leftarrow \text{factor}(\code{indu[$\alpha$]$\tau$})
    \\
    ...
    %% right-induction
    \I \B{case } \tau_l, \code{indu[$\alpha$]$\tau$} :
    \\
