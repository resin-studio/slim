...
% an interpolant of an implication is akin to a subtype of a function type
% X -> Y \sqsubseteq A -> B === A, B |- [X -> Y] == A \sqsubseteq X, Y \sqsubseteq B

% constructing an interpolant from resolution is akin finding subtype that unifies with a function type 

% an inductive invariant from interpolants is akin to a nu-type from component types of the fix rule 
