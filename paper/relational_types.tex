...
\begin{flalign*}
  &\boxed{\left<\Delta, \Gamma, \tau\right> \in \llbracket e : \tau \rrbracket^\sharp}&
\end{flalign*}
\begin{mathpar}

  \inferrule { 
    \Delta \in \llbracket \bullet \vdash (\Gamma\ x) \leq \tau \rrbracket^\sharp
  } {
  ...
  \inferrule { 
    \Delta \in \llbracket \bullet \vdash \text{unit} \leq \tau \rrbracket^\sharp
  } {
  ...
  \inferrule { 
    \text{fresh}\ \alpha
    \\
    \Delta \in \llbracket \bullet \vdash (l * \alpha) \leq \tau \rrbracket^\sharp
    \\
  ...
  \inferrule { 
    \text{fresh}\ \alpha_i
    \\
    \Delta \in \llbracket \bullet \vdash (\wedge\ \widebar{l_i : \alpha_i}) \leq \tau \rrbracket^\sharp
    \\
  ...
  \inferrule { 
    ...
    \text{fresh}\ \alpha_i''
    \\
    \Delta \in \llbracket \bullet \vdash (\wedge\ \widebar{\alpha'_i \rightarrow \alpha''_i}) \leq \tau \rrbracket^\sharp
    \\
  ...
  \inferrule { 
    ...
    \text{fresh}\ \alpha
    \\
    \Delta \in \llbracket \bullet \vdash \tau' \leq (l : \alpha) \rrbracket^\sharp
  } {
  ...
  \inferrule { 
    ...
    \text{fresh}\ \alpha'
    \\
    \Delta \in \llbracket \bullet \vdash \tau_1 \leq \tau_2 \rightarrow \alpha' \rrbracket^\sharp
    \\
    \text{fresh}\ \alpha''
    \\
    % need to call unification one more time to liberate existentially quantified variables 
    \Delta \in \llbracket \bullet \vdash \alpha'' \leq \alpha' \rrbracket^\sharp
  } {
    \left<\Delta, \Gamma, \alpha'' \right>
  ...
  \inferrule { 
    ...
    \text{fresh}\ \alpha_2
    \\
    \Delta \in \llbracket \bullet \vdash \tau' \leq \alpha_1 \rightarrow \alpha_2 \rrbracket^\sharp
  } {
...
% \begin{figure*}
% \begin{flalign*}
%   &\boxed{\Delta \vdash \tau \triangledown \tau = \tau}&
% \end{flalign*}
% \[
% \begin{array}{l @{}l}
%   & {}=
...
% RelType's termination is based on structural recursion modulo the type variables rather than monotonicity of types.
...
\begin{mathpar}
  ...
  \inferrule {
    ...
    (\Delta, \alpha \mapsto (\text{rolldn}\ \alpha\ (\tau' \wedge \tau))) 
    \in \llbracket \Omega \vdash \alpha \leq \tau \rrbracket^\sharp
  }
  ...
  \inferrule {
    ...
    (\Delta, \alpha \mapsto (\text{rollup}\ \alpha\ (\tau' \vee \tau))) 
    \in \llbracket \Omega \vdash \tau \leq \alpha \rrbracket^\sharp
  }
