...
\begin{figure*}[h]
\begin{flalign*}
  &\boxed{\text{case}\ \alpha\ \tau\ \tau'}&
\end{flalign*}
\[
\begin{array}{l @{}l}
  \text{case}\ \alpha\ \tau\ \tau'
  & {}=  
  \begin{cases}
    \left<\forall \widebar{\alpha_i}^i\ .\ \tau'\ |\ \alpha \sqsubseteq \tau \right> 
    &\text{if}\ \bigwedge_i\ \alpha_i \in (\text{freevars}\ \tau') \wedge (\bigvee_j\  i = j \wedge \alpha_j \in (\text{freevars}\ \tau))
    \\
    \forall \widebar{\alpha_i}^i\ .\ \tau'
    &\text{if}\ \bigwedge_i\ \alpha_i \in (\text{freevars}\ \tau') 
    \\
    \tau' &\text{otherwise}
  \end{cases}
\end{array}
