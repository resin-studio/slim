...
% construct a least fixed point search that uses widening of two types (related by subtyping)
% ditto for narrowing
% parameter types are default to Top and and narrow based on internal usage.
% parameter types default to Top and widen based on external usage. 
% widening operator could depend on Type environment for type variables
% separate F : L -> L from Widen : L * L -> L 
% F replaces iteration/recursion of unification step 
% widening replaces failure decision within step
\begin{flalign*}
  &\boxed{\left<\Delta, \Gamma, \tau\right> \in \llbracket x \mapsto e : \tau \rrbracket\#}&
\end{flalign*}
\begin{mathpar}
  ...
  \inferrule { 
    ...
    \in \llbracket e : \alpha \rrbracket\#
  } {
    \left< \Delta, \Gamma, (l * \tau')\right> 
    \in \llbracket (l \cdot e) : \tau \rrbracket\#
  } 
  ...
  \inferrule { 
    ...
    \in  \llbracket e_i : \alpha_i \rrbracket\#
  } {
    \left< \Delta, \Gamma, (\wedge\ \widebar{l_i : \tau_i}) \right> 
    \in \llbracket (\sigma\ \widebar{l_i = e_i}) : \tau \rrbracket\#
  } 
  ...
  \inferrule { 
    ...
    \in \llbracket m_i : \alpha'_i \rrbracket\#
    \\
    \left< \Delta, \Gamma, \tau''_i \right> 
    \in \llbracket e_i : \alpha''_i \rrbracket\#
  } {
    \left< \Delta, \Gamma, (\wedge\ \widebar{(\tau_i' \rightarrow \tau_i''}) \right>
    \in
    \llbracket (\lambda\ \widebar{m_i \Rightarrow e_i}) : \tau \rrbracket\#
  } 

  \inferrule { 
    \left<\Delta, \Gamma, \tau' \right>
    \in \llbracket e : (l : \tau) \rrbracket\#
    \\
    \text{fresh}\ \alpha
    \\
  ...
  \inferrule { 
    ...
    \in \llbracket e_1 : (\alpha \rightarrow \tau) \rrbracket\#
    \\
    \left< \Delta, \Gamma, \tau_2 \right> 
    \in \llbracket e_2 : \alpha \rrbracket\#
    \\\\
    \text{fresh}\ \alpha'
    \\
    ...
    \left<\Delta, \Gamma, \alpha'' \right>
    \in \llbracket (e_1\ e_2) : \tau \rrbracket\#
  } 

  \inferrule { 
    \left< \Delta, \Gamma, \tau_1' \right> 
    \in \llbracket e_1 : \tau_1 \rrbracket\#
    \\
    \tau_1'[\Delta] = \tau_1'
    \\
    ...
    \in \llbracket e_2 : \tau_2 \rrbracket\#
    \\
  } {
    \left< \Delta, \Gamma, \tau_2' \right> 
    \in \llbracket (\text{let}\ x : \tau_1 = e_1\ \text{in}\ e_2) : \tau_2 \rrbracket\#
  } 

  \inferrule { 
    \left< \Delta, \Gamma, \tau' \right> 
    \in \llbracket e : \bot \rightarrow \tau \rrbracket\#
    \\
    \text{fresh}\ \alpha_1
    \\
    \text{fresh}\ \alpha_2
    ...
    \in \llbracket \text{fix}\ e : \tau \rrbracket\#
  } 
\end{mathpar}
\end{definition}
