...
\begin{mathpar}
  ...
  \inferrule[Proj] {
    ...
    (\omega\ \widebar{l_i \approx v_i})/l_j \leadsto v_j
  } 
...
\begin{mathpar}
  \inferrule { 
    \forall e'\ .\ e \leadsto^* e' \longrightarrow (e' = v \vee e' \leadsto e'') 
  } {
...
\begin{mathpar}
  ...
  %%%%%%%%%%%%%
  %% here are some over-engineered definitions and lemmas in order to match absint framework
  %%%%

  \inferrule { 
  } {
    \left< E \right> = \text{reduce} \wedge\ \{\left< \cdot \vdash e \right>\ |\ e \in E \}
  } 
  ...
  \\\\
  %%%%%%%%%%%%%%%%%%%%%%
  %% here is a simplified galois connection, but not as general as absint framework
  %%%%%%%%%%%%%%
  %% TODO: consider how widening would affect definition of typing.
  %% completed type would correspond with value, not partial expression.
  %% partial expressions do not have smaller type than resulting value.
  % thoughts on abstract interpretation
    % where it's just over-engineering
      % for natural deduction, concretizing is only needed for soundness proof.
      ...
      % defining concretizing and proving a galois connection
        % is in effect the same as proving soundness 
    % where it's valuable
      % for transition systems that widen/narrow with each step,
        % concretizing enables leveraging transitions of the dynamic semantics
      % therefore enables method for construction of small-step static semantics 

  \inferrule { 
  } {
    \left< \cdot \vdash e \right> \leq \tau
    \equiv
    \llbracket e \rrbracket \subseteq \lfloor \tau \rfloor
  } 
