...
\begin{figure*}[h]
  \begin{mathpar}
    ...
    \inferrule[Proj] {
      ...
      \omega [\hdots, l \approx v, \hdots]/l \longrightarrow v
    } 
...
\begin{figure*}[h]
  \begin{mathpar}
    ...
    \inferrule[Record] { 
      \bigwedge_{l\sim\tau} 
      \bigwedge_{l \approx t} 
      \Gamma \vdash t : \tau
    } {
      \Gamma \vdash \omega\ (\widebar{l \approx t}) : (\text{reduce} \wedge \widebar{l\sim\tau})  
    } 
...
\begin{figure*}[h]
  \[
    \begin{array}{l @{} l}
      ...
      \text{infer } \iota\ \Delta\ \Gamma \vdash 
      \omega\ \widebar{l \approx t} : \tau
      &{} =
      \begin{array}[t]{@{} l}
        \text{let } \widebar{(l \approx t, \alpha)} = 
        [(l \approx t, \text{typevar}()) \ |\ l \approx t ]
        \\
        \text{let } \tau_1 = \text{reduce} \wedge [l \sim \alpha \ |\ (l \approx t, \alpha) ]
        \\
        \text{reduce} * \{
      ...
      \text{infer } \iota\ \Delta\ \Gamma \vdash 
      \lambda\ \widebar{p : \tau \Rightarrow t} : \tau
      &{} =
      \begin{array}[t]{@{} l}
        \text{let } \widebar{(p : \tau \Rightarrow t, \alpha)} = 
        [(p : \tau \Rightarrow t, \text{typevar}()) \ |\ p : \tau \Rightarrow t]
        \\
        \text{let } \tau_1 = \text{reduce} \wedge 
        [\tau \rightarrow \alpha \ |\ (p : \tau \Rightarrow t, \alpha)]
        \\
        \text{reduce} * \{
        ...
        \ \ \ \ \Delta_1 \in \text{unify } \iota\ \Delta\ \vdash \tau_1 \leq \tau
        \\
        \ \ \ \ (p : \tau \Rightarrow t, \alpha)
        \\
        \ \ \ \ \Gamma_1 \in \{\text{patenv } p\}
        \\
        \ \ \ \ \Delta_2.\tau' \in \text{infer } \blacksquare\ \Delta;\Delta_1\ \Gamma;\Gamma_1\ \vdash p : \tau
        \\
        \}
        \\
      \end{array}
...
\begin{figure*}[h]
  \[
    %TODO: move quantifier cases to the beginning 
    %TODO: remove quantifier variable cases
    \begin{array}{l @{} l}
...
\begin{figure*}[h]
  \[
    ...
    &\ = \top
    \\
    \text{reduce} \wedge \tau_1::\widebar{\tau_2}
    &\ = \tau_1\ \wedge (\text{reduce} \wedge \widebar{\tau_2})
    \end{array}
  \]
...
\begin{figure*}[h]
  \[
    ...
    & {}= N
    \\
    \text{reduce} * N_1::\widebar{N_2}
    & {}= N_1 * (\text{reduce} * \widebar{N_2})
    \end{array}
  \]
...
% curious idiosyncrasy
% let-polymorphism with subtyping allows expected type to be smaller than checked type
  % in particular case of universal <: existential
  % this is due to the fact that a particular term inhabiting a universal
  % must be specialized at time of use.
...
% unrolling corresponds to iteratively finding least/greatest fixedpoint 
  % recursion on rhs; find least fixedpoint
  % co-recursion on lhs; find greatest fixedpoint
% Abstract interpretations' widening/narrowing operator is embedded 
  % recursion: if not unroll-form  then failure, corresponding to lhs widening to top
  % co-recursion: if not unroll-form then failure, corresponding to rhs narrowing to bottom 
