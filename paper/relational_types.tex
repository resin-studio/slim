...
\textit{Set-theoretic types} \cite[Castagna et al]{?} offer ways to describe programs statically 
with union and intersection type combinators (among other set-related features). 
In contrast to refinement types, where a term constructor is prescribed a particular top type by the datatype mechanism,
...
with some additional costs of annotation. Liquid types combine Hindley-Milner type inference 
...
reconstruct types by propagating type information up. \text{roundtrip typing} \cite[?]{} is an extension of 
...
Additionally, the combination of upward and downward propagation of types makes type checking more efficient 
by pruning the search space, much like how CDCL and BCP combine to prune the search space in SAT. 

\textbf{TODO:} think about connection between between type adjustment and predicate abstraction.
\newline
...
\begin{mathpar}
  \inferrule { 
    \Delta \cdot \bullet \Vdash e : \tau \leq \top
  } {
...
\begin{mathpar}
  ...
  \inferrule { 
    \text{fresh}\ \alpha
    \\
    \Delta \cdot \bullet \Vdash (l * \alpha) \leq \tau
    \\
    \Delta;\Delta' \cdot \Gamma
    \Vdash e : \tau' \leq \alpha
  } {
    \Delta;\Delta' \cdot \Gamma 
    \Vdash (l \cdot e) : (l * \tau') \leq \tau
  } 

  \inferrule { 
    \text{fresh}\ \alpha_i
    \\
    \Delta \cdot \bullet \Vdash (\wedge\ \widebar{l_i : \alpha_i}) \leq \tau
    \\
    \Delta;\Delta' \cdot \Gamma
    \Vdash e_i : \tau_i \leq \alpha_i
  } {
    \Delta;\Delta' \cdot \Gamma
    \Vdash (\sigma\ \widebar{l_i = e_i}) : 
    (\wedge\ \widebar{l_i : \tau_i}) \leq \tau
  } 
  ...
  \inferrule { 
    ...
    \text{fresh}\ \alpha_i''
    \\
    \Delta \cdot \bullet \Vdash (\wedge\ \widebar{\alpha'_i \rightarrow \alpha''_i}) \leq \tau
    \\
    \Delta;\Delta' \cdot \Gamma;(\text{patenv}\ m_i) 
    \Vdash m_i : \tau'_i \leq \alpha'_i
    \\
    \Delta;\Delta';\Delta'' \cdot \Gamma 
    \Vdash e_i : \tau''_i \leq \alpha''_i
  } {
    \Delta;\Delta';\Delta'' \cdot \Gamma
    \Vdash
    (\lambda\ \widebar{m_i \Rightarrow e_i}) : 
    (\wedge\ \widebar{\tau_i' \rightarrow \tau_i''}) \leq 
    \tau
  } 
  ...
  \inferrule { 
    ...
    \text{fresh}\ \alpha
    \\
    \Delta;\Delta' \cdot \bullet \Vdash \tau' \leq (l : \alpha)
  } {
    \Delta;\Delta' \cdot \Gamma
    \Vdash (e/l) : \alpha \leq \tau
  } 
  ...
  \inferrule { 
    ...
    \\
    \Delta \cdot \Gamma 
    \Vdash e_1 : \tau_1 \leq (\alpha \rightarrow \tau)
    \\
    \Delta;\Delta' \cdot \Gamma
    \Vdash e_2 : \tau_2 \leq \alpha
    \\
    \text{fresh}\ \alpha'
    \\
    \Delta;\Delta';\Delta'' \cdot \bullet \Vdash \tau_1 \leq \tau_2 \rightarrow \alpha'
    \\
    \text{fresh}\ \alpha''
    \\
    % need to call unification one more time to liberate existentially quantified variables 
    \Delta;\Delta';\Delta'';\Delta''' \cdot \bullet \Vdash \alpha'' \leq \alpha'
  } {
    \Delta;\Delta';\Delta'';\Delta''' \cdot \Gamma
    \Vdash (e_1\ e_2) : \alpha'' \leq \tau
  } 

  \inferrule { 
    \Delta \cdot \Gamma 
    \Vdash e_1 : \tau_1' \leq \tau_1
    \\
    \Delta;\Delta' \cdot (\Gamma, x \mapsto \text{pack}\ \Delta\ \tau_1')  
    \Vdash e_2 : \tau_2' \leq \tau_2
    \\
  } {
    \Delta;\Delta' \cdot \Gamma 
    \Vdash 
  ...
  \inferrule { 
    ...
    \\
    \Delta \cdot \Gamma 
    \Vdash e : \tau' \leq \alpha_1 \rightarrow \tau
    \\
    \text{fresh}\ \alpha_2
    \\
    \Delta;\Delta' \cdot \bullet \Vdash \tau' \leq \alpha_1 \rightarrow \alpha_2
    \\\\
    \alpha_1[\Delta] = \tau_1
    \\
    \alpha_2[\Delta] = \tau_2
    ...
    \Delta;\Delta' \cdot \Gamma  
    \Vdash \text{fix}\ e :(\nu\ \alpha_3\ .\  \wedge_i(\widebar{\text{twist}\ \alpha_2\ \tau_1\ \tau_i'})) 
    \leq \tau
  } 
...
\begin{mathpar}
  \inferrule {
    \alpha_i \not\in (\text{dom}\ \Delta)
    \\
    \Delta;\Delta' \cdot \Omega \Vdash \tau_1 \leq \tau_2
    \\
    \Delta;\Delta';\Delta'' \cdot \Omega \Vdash \tau_3 \leq \tau_4
  } {
  ...
  \inferrule {
    (\Delta\ \alpha) = \tau'
    \\
    (\Delta;\Delta') \cdot \Omega \Vdash \tau' \leq \tau
  } {
  ...
  \inferrule {
    (\Delta\ \alpha) = \tau'
    \\
    \neg (\Delta;\Delta' \cdot \Omega \Vdash \tau' \leq \tau)
  } {
  ...
  \inferrule {
    ...
    \Delta;\Delta' \cdot \Omega \Vdash \tau \leq \alpha
  }
  ...
  \inferrule {
    ...
    \\\\
    \Delta;\Delta' \cdot \Omega, k \mapsto \tau_4 \Vdash \tau_1 \leq \tau_2
    \\
  ...
  \inferrule {
    ...
    \\\\
    \Delta;\Delta' \cdot \Omega \Vdash \tau_3 \leq \tau_4
    \\
    \Delta;\Delta';\Delta'' \cdot \Omega \Vdash \tau_1 \leq \tau_2
    \\
    % safety: bound vars can only assign to bound vars or non-vars
    \text{safe}\ \widebar{\alpha_i}\ \Delta''
  } {
...
\begin{mathpar}
  \inferrule {
    ...
    \Delta \cdot \Omega \Vdash \text{unit} \leq \text{unit}
  }

  \inferrule {
    \Delta \cdot \Omega \Vdash \tau_1 \leq \tau_2
  } {
    \Delta \cdot \Omega \Vdash l*\tau_1 \leq l*\tau_2
  }
  ...
  \inferrule {
    \Delta \cdot \Omega \Vdash \tau_3 \leq \tau_1
    \\
    \Delta;\Delta' \cdot \Omega \Vdash \tau_2 \leq \tau_4
  } {
    \Delta;\Delta' \cdot 
  ...
  \inferrule {
    ...
    \Delta \cdot \Omega \Vdash \mu \alpha_1 . \tau_1 \leq \mu \alpha_2 . \tau_2
  }
  ...
  \inferrule {
    ...
    \text{complex}\ \tau_1 = k 
    \\
    \Delta \cdot \Omega \Vdash (\Omega\ k) \leq \mu \alpha . \tau_2
  } {
    \Delta \cdot \Omega \Vdash \tau_1 \leq \mu \alpha . \tau_2
  }
  ...
  \inferrule {
    ...
    \text{complex}\ \tau_1 \neq k 
    \\
    \Delta \cdot \Omega \Vdash \tau_1 \leq \tau_2[\alpha \mapsto \mu \alpha . \tau_2]
  } {
    \Delta \cdot \Omega \Vdash \tau_1 \leq \mu \alpha . \tau_2
  }
  ...
  \inferrule {
    \Delta \cdot \Omega \Vdash \tau_1 \leq \tau_3
    \\
    \Delta;\Delta' \cdot \Omega \Vdash \tau_2 \leq \tau_3
  } {
  ...
  \inferrule {
    \Delta \cdot \Omega \Vdash \tau_1 \leq \tau_2
    \\
    \Delta;\Delta' \cdot \Omega \Vdash \tau_1 \leq \tau_3
  } {
  ...
  \inferrule {
    \Delta \cdot \Omega \Vdash \tau_2 \leq \tau_3
  } {
    \Delta \cdot \Omega \Vdash \tau_1 \wedge \tau_2 \leq \tau_3
  }
\end{mathpar}
...
% \item Set-theoretic types (Castagna) \url{https://www.irif.fr/~gc/papers/set-theoretic-types-2022.pdf}
