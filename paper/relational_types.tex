...
\begin{mathpar}
  ...
  \inferrule { 
    ...
    \Gamma \vdash \omega\ (\widebar{l_i \approx t_i})
  ...
  \inferrule {
    ...
    \Gamma \vdash \lambda\ (\widebar{p_i \Rightarrow t_i})
...
\begin{mathpar}
  ...
  \inferrule { 
    ...
    \Delta \Vdash \left<\tau_3 :: \exists \widebar{\alpha_i}\ .\ \tau_1 \leq \tau_2\right>
    \leq \tau_4
  }
  ...
  \inferrule { 
    ...
    \Delta \Vdash \tau_1
    \leq \left<\tau_4 :: \exists \widebar{\alpha_i}\ .\ \tau_2 \leq \tau_3\right>
  }
  ...
  \inferrule { 
    ...
    \Delta \Vdash \tau_1
    \leq \left<\tau_4 :: \forall \widebar{\alpha_i}\ .\ \tau_2 \leq \tau_3\right>
  }
  ...
  \inferrule { 
    ...
    \Delta \Vdash \left<\tau_3 :: \forall \widebar{\alpha}\ .\ \tau_1 \leq \tau_2\right>
    \leq \tau_4
  }
...
% comparison to model checking
  ...
  % union/intersection adjustment is a more surgical version of 
    % (widening,expansion)/(narrowing,contraction)
      % the latter of which is wrt inductive invariants (IH)
    % abstract interpretation imposes decision condition from outside
    % type adjustment decides based on previous state
...
% motivation of abstract interpretation
  % widening/narrowing
    % decision when to widen/narrow imposed from outside
    % differs from type adjustment, which decides to widen/narrow from info in previous state
  % where it's valuable
    % abstract interpretation is motivated by data-flow
    % data-flow is motivated by side effects
    % notes:
