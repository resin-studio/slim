...
% main contributions 
  % challenge: instantiation of polymorphic arguments to narrow search
    % sota: separate instantiation and subtyping
    % novel solution: partial instantiation in generalized polymorphic subtyping
      % subtyping encodes instantiation: ((ALL X . T) <: T')
  % challenge: synthesis from context (without annotations) 
    % sota: bidirectional/roundtrip typing using annotations 
    % novel solution: both up and down propagation for every syntax rule
      % propagating types (downward/upward) 
      % A synthesis algorithm capable of generating local constraints for incomplete programs. 
  % challenge: safe but lenient type inference without prescription
    % sota: datatypes/ML languages prescribe upper bound 
    % novel solution: adjusting types with union and intersection
      % A unification algorithm capable of preserving the flexibility of dynamically typed programming. 
  % challenge: inference of expressive types
    % sota: refinement types require base type 
    % novel solution: relational types
      % relating types (recursive/corecursive) 
...
% additional observations 
  % observation: type inference without constraints 
    % sota: separate constraints and types
      % constraint type schemes are separate from subtyping
      % constraint language is more expressive than type language
    % novel solution: 
      % constraint semantics are subsumed by subtyping
      % subtyping is expressive enough to constrain acceptable programs  
