...
\begin{mathpar}
  ...
  \inferrule {
    ...
    (\sigma\ \widebar{l_i = v_i})/l_j \leadsto v_j
  } 

  \inferrule { 
  } {
    (\lambda\ \widebar{p \Rightarrow t})\ v 
    \leadsto 
    (\text{match } v\ \widebar{p \Rightarrow t})
  } 
  ...
  \inferrule { 
    ...
    \text{fix}\ \lambda [x \Rightarrow t]
    \leadsto 
    t[x \mapsto \text{fix}\ \lambda [x \Rightarrow t]]
  } 
...
\begin{definition}
\begin{flalign}
  &\boxed{\text{match}\ v\ (\widebar{p \Rightarrow t}) = H}&
\end{flalign}
\[
  \begin{array}{l @{} l}
    \text{match}\ v\ ((p \Rightarrow t) :: cs)
    &{} =
    \begin{cases}
      t[H]
      &\text{if }
...
\[
  \begin{array}{l @{} l}
    ...
    &{} =
    \text{reduce} \cdot \widebar{\text{unify}\ v\ p}
  \end{array}
\]
\end{definition}
...
\begin{flalign}
  &\boxed{\left<\kappa, \rho, e \right> \leadsto \left<\kappa, \rho, e \right>}&
\end{flalign}
\begin{mathpar}
  \inferrule { 
  } {
    \left<\left<\rho_\kappa, x, e\right> :: \kappa, \rho, v \right> 
    \leadsto 
    \left<\kappa, (\rho_\kappa,x \mapsto \left<\rho,v\right>), e \right>
  } 

  \inferrule { 
    x = \text{fresh}\ ()
  } {
    \left<\kappa, \rho, l;e \right> 
    \leadsto 
    \left<
      \left< \rho, x, (l \cdot x)\right> :: \kappa, 
      \rho,
      e 
    \right>
  } 
...
\begin{definition}(Abstract state)
\end{definition}
...
\begin{definition}[Type transition]
\begin{mathpar}
  \inferrule { 
    \bullet \vdash e : \tau
    \\
    ...
    \left<e, \tau\right> \rightharpoonup \left<e', \tau'\right>
  } 
\end{mathpar}
\end{definition}

\begin{theorem}(Type transition progress)
\begin{mathpar}
  \inferrule { 
    \bullet \vdash e : \tau
    \\
...
\begin{theorem}(Type transition preservation)
\begin{mathpar}
  \inferrule { 
    \bullet \vdash e : \tau
    \\
...
% motivation of abstract interpretation
  % where it's valuable
    ...
    % data-flow is motivated by side effects/(widening or narrowing)
    % notes:
