...
\begin{flalign*}
  &\boxed{\left<\kappa, \rho, c \right> \hookrightarrow \left<\kappa, \rho, c \right>}&
\end{flalign*}
\begin{mathpar}
  \inferrule { 
  } {
  ...
  \inferrule { 
    \text{contify}\ e = (x_\kappa, e_\kappa, e')
  } {
  ...
  \inferrule { 
    ...
    \left< \kappa, \rho, \text{let}\ x : \tau = p_1\ \text{in}\ p_2 \right> 
    \hookrightarrow 
...
\begin{mathpar}
  ...
  % \inferrule { 
  %   \left<e, \rho, \kappa \right> \hookrightarrow^* \varsigma 
  %   \\
  %   \varsigma = \left<w, \rho, \circ \right>
  % } {
  %   \left<e, \rho, \kappa \right> \in \llbracket e \rrbracket\flat
  % } 

  % \inferrule { 
  %   \forall \varsigma \cdot \left<e, \rho, \kappa \right> \hookrightarrow^* \varsigma 
  %   \longrightarrow (\varsigma \hookrightarrow \varsigma' \vee \varsigma = \left<w, \rho, \circ \right>)
  % } {
  %   \left<e, \rho, \kappa \right> \in \llbracket e \rrbracket\flat
  % } 
\end{mathpar}
...
\begin{flalign*}
  &\boxed{\left<\Delta, \Gamma, \tau\right> \in \llbracket x \mapsto e : \tau \rrbracket^\sharp}&
\end{flalign*}
\begin{mathpar}

  \inferrule { 
    \Delta \in \llbracket (\Gamma\ x) \leq \tau \rrbracket^\sharp
  } {
  ...
  \inferrule { 
    \Delta \in \llbracket \text{unit} \leq \tau \rrbracket^\sharp
  } {
    \left< \Delta, \Gamma, \text{unit} \right> 
  ...
  \inferrule { 
    \text{fresh}\ \alpha
    \\
    \Delta \in \llbracket (l * \alpha) \leq \tau \rrbracket^\sharp
    \\
    \left< \Delta, \Gamma, \tau' \right> 
    \in \llbracket e : \alpha \rrbracket^\sharp
  } {
  ...
  \inferrule { 
    \text{fresh}\ \alpha_i
    \\
    \Delta \in \llbracket (\wedge\ \widebar{l_i : \alpha_i}) \leq \tau \rrbracket^\sharp
    \\
    \left<\Delta, \Gamma, \tau_i\right> 
    \in  \llbracket e_i : \alpha_i \rrbracket^\sharp
  } {
    \left< \Delta, \Gamma, (\wedge\ \widebar{l_i : \tau_i}) \right> 
    \in \llbracket (\sigma\ \widebar{l_i = e_i}) : \tau \rrbracket^\sharp
  } 
  ...
  \inferrule { 
    ...
    \text{fresh}\ \alpha_i''
    \\
    \Delta \in \llbracket (\wedge\ \widebar{\alpha'_i \rightarrow \alpha''_i}) \leq \tau \rrbracket^\sharp
    \\\\
    \left<\Delta, \Gamma;(\text{patenv}\ m_i), \tau'_i\right> 
    \in \llbracket m_i : \alpha'_i \rrbracket^\sharp
    \\
    \left< \Delta, \Gamma, \tau''_i \right> 
    \in \llbracket e_i : \alpha''_i \rrbracket^\sharp
  } {
    \left< \Delta, \Gamma, (\wedge\ \widebar{(\tau_i' \rightarrow \tau_i''}) \right>
    \in
    \llbracket (\lambda\ \widebar{m_i \Rightarrow e_i}) : \tau \rrbracket^\sharp
  } 

  \inferrule { 
    \left<\Delta, \Gamma, \tau' \right>
    \in \llbracket e : (l : \tau) \rrbracket^\sharp
    \\
    \text{fresh}\ \alpha
    \\
    \Delta \in \llbracket \tau' \leq (l : \alpha) \rrbracket^\sharp
  } {
  ...
  \inferrule { 
    \text{fresh}\ \alpha
    \\
    \left< \Delta, \Gamma, \tau_1 \right> 
    \in \llbracket e_1 : (\alpha \rightarrow \tau) \rrbracket^\sharp
    \\
    \left< \Delta, \Gamma, \tau_2 \right> 
    \in \llbracket e_2 : \alpha \rrbracket^\sharp
    \\\\
    \text{fresh}\ \alpha'
    \\
    \Delta \in \llbracket \tau_1 \leq \tau_2 \rightarrow \alpha' \rrbracket^\sharp
    \\
    \text{fresh}\ \alpha''
    \\
    % need to call unification one more time to liberate existentially quantified variables 
    \Delta \in \llbracket \alpha'' \leq \alpha' \rrbracket^\sharp
  } {
    \left<\Delta, \Gamma, \alpha'' \right>
    \in \llbracket (e_1\ e_2) : \tau \rrbracket^\sharp
  } 

  \inferrule { 
    \left< \Delta, \Gamma, \tau_1' \right> 
    \in \llbracket e_1 : \tau_1 \rrbracket^\sharp
    \\
    \left< \Delta, (\Gamma, x = \text{pack}\ \Delta\ \tau_1'), \tau_2' \right>  
    \in \llbracket e_2 : \tau_2 \rrbracket^\sharp
    \\
  } {
    \left< \Delta, \Gamma, \tau_2' \right> 
    \in \llbracket (\text{let}\ x : \tau_1 = e_1\ \text{in}\ e_2) : \tau_2 \rrbracket^\sharp
  } 
  ...
  \inferrule { 
    ...
    \\
    \left< \Delta, \Gamma, \tau' \right> 
    \in \llbracket e : \alpha_1 \rightarrow \tau \rrbracket^\sharp
    \\
    \text{fresh}\ \alpha_2
    \\
    \Delta \in \llbracket \tau' \leq \alpha_1 \rightarrow \alpha_2 \rrbracket^\sharp
  } {
    \left< \Delta, \Gamma, (\text{spiral}\ \Delta\ \alpha_1\ \alpha_2) \right>
    \in \llbracket \text{fix}\ e : \tau \rrbracket^\sharp
  } 
\end{mathpar}
\end{definition}
...
\begin{definition}[Subtype itermediate unification ]
...
\begin{mathpar}
  ...
  \inferrule {
    \alpha_i \not\in (\text{dom}\ \Delta)
    \\
    \Delta;\Delta' \in \llbracket \Omega \vdash \tau_1 \leq \tau_2 \rrbracket^\sharp
    \\
    \Delta;\Delta';\Delta'' \in \llbracket \Omega \vdash \tau_3 \leq \tau_4 \rrbracket^\sharp
  } {
    \Delta \in \llbracket 
      \Omega \vdash  
      (\forall \widebar{\alpha_i}\ .\ \tau_1\ |\ \tau_3 \leq \tau_4)
      \leq \tau_2
    \rrbracket^\sharp
  }
  ...
  \inferrule {
    ...
    \Delta \in \llbracket 
      ...
      (\exists \widebar{\alpha_i}\ .\ \tau_2\ |\ \tau_3 \leq \tau_4)
    \rrbracket^\sharp
  }
  ...
  \inferrule {
    ...
    (\Delta, \alpha \mapsto \text{rolldn}\ \alpha\ \tau) 
  ...
  \inferrule {
    (\Delta\ \alpha) = \tau'
    \\
    (\Delta;\Delta') \in \llbracket \Omega \vdash \tau' \leq \tau \rrbracket^\sharp
  } {
    (\Delta;\Delta') \in \llbracket \Omega \vdash \alpha \leq \tau \rrbracket^\sharp
  }

  \inferrule {
    (\Delta\ \alpha) = \tau'
    \\
    \llbracket \Omega \vdash \tau' \leq \tau \rrbracket^\sharp \subseteq \emptyset
  } {
    (\Delta, \alpha \mapsto \tau' \blacktriangle (\text{rolldn}\ \alpha\ \tau)) 
    \in \llbracket \Omega \vdash \alpha \leq \tau \rrbracket^\sharp
  }
  \\\\
  \inferrule {
    ...
    \in \llbracket \Omega \vdash \tau \leq \alpha \rrbracket^\sharp
  }
  ...
  \inferrule {
    ...
    (\Delta;\Delta') \in \llbracket \Omega \vdash \tau \leq \alpha \rrbracket^\sharp
  }
  ...
  \inferrule {
    ...
    (\Delta, \alpha \mapsto \tau' \blacktriangledown (\text{rollup}\ \alpha\ \tau)) 
    \in \llbracket \Omega \vdash \tau \leq \alpha \rrbracket^\sharp
  }
  ...
  \inferrule {
    ...
    \\
    \Delta;\Delta' \in \llbracket \Omega, k \mapsto \tau_4 \vdash \tau_1 \leq \tau_2 \rrbracket^\sharp
    \\
    ...
    \Delta \in \llbracket 
      \Omega \vdash 
      (\exists \widebar{\alpha_i}\ .\ \tau_1\ |\ \tau_3 \leq \tau_4) \leq
      \tau_2
    \rrbracket^\sharp
  }
  ...
  \inferrule {
    ...
    \\
    \Delta;\Delta' \in \llbracket \Omega \vdash \tau_3 \leq \tau_4 \rrbracket^\sharp
    \\\\
    \Delta;\Delta';\Delta'' \in \llbracket \Omega \vdash \tau_1 \leq \tau_2 \rrbracket^\sharp
    \\
    % safety: bound vars can only assign to bound vars or non-vars
    \text{safe}\ \widebar{\alpha_i}\ \Delta''
  } {
  ...
  \inferrule {
    ...
    \Delta \in \llbracket 
      ...
      \tau_1
      \leq
      (\forall \widebar{\alpha_i}\ .\ \tau_2\ |\ \tau_3 \leq \tau_4)
    \rrbracket^\sharp
  }
  ...
  \inferrule {
    ...
    \Delta \in \llbracket 
      \Omega \vdash \text{unit} \leq \text{unit}
    \rrbracket^\sharp
  }
  ...
  \inferrule {
    \Delta \in \llbracket \Omega \vdash \tau_3 \leq \tau_1 \rrbracket^\sharp
    \\
    \Delta;\Delta' \in \llbracket \Omega \vdash \tau_2 \leq \tau_4 \rrbracket^\sharp
  } {
  ...
  \inferrule {
    ...
    \\
    \Delta \in \llbracket 
      \Omega \vdash (\Omega\ k) \leq \mu \alpha . \tau_2
    \rrbracket^\sharp
  } {
    \Delta \in \llbracket 
  ...
  \inferrule {
    ...
    \\
    \Delta \in \llbracket 
      \Omega \vdash \tau_1 \leq \tau_2[\alpha \mapsto \mu \alpha . \tau_2]
    \rrbracket^\sharp
  } {
    \Delta \in \llbracket 
      \Omega \vdash \tau_1 \leq \mu \alpha . \tau_2
...
% \begin{flalign*}
%   &\boxed{\Delta \cdot \Gamma \models x \mapsto e : \tau}&
% \end{flalign*}
% \begin{mathpar}
%   \inferrule { 
%     \Delta \models (\Gamma\ x) \leq \tau
%   } {
%     \Delta \cdot \Gamma \models  x : \tau
%   } 

%   \inferrule { 
%     \Delta \models \text{unit} \leq \tau
%     \\
%     \text{unit} \sqsupseteq (\Gamma\ x)
%   } {
%     \Delta \cdot \Gamma \models x \mapsto () : \tau
%   } 

%   \inferrule { 
%     \text{fresh}\ \alpha
%     \\
%     \Delta \models (l * \alpha) \leq \tau
%     \\\\
%     \Delta \cdot \Gamma \cdot \models x' \mapsto e : \alpha
%     \\
%     (l * (\Gamma\ x')) \sqsupseteq (\Gamma\ x)
%   } {
%     \Delta\ \cdot \Gamma \models x \mapsto (l \cdot e) : \tau
%   } 

%   \inferrule { 
%     \text{fresh}\ \alpha_i
%     \\
%     \Delta \models (\wedge\ \widebar{l_i : \alpha_i}) \leq \tau
%     \\\\
%     \Delta \cdot \Gamma \models x_i \mapsto e_i : \alpha_i
%     \\
%     (\wedge\ \widebar{l_i : (\Gamma\ x_i)}) \sqsupseteq (\Gamma\ x)
%   } {
%     \Delta\ \cdot \Gamma \models x \mapsto (\sigma\ \widebar{l_i = e_i}) : \tau
%   } 
...
%     \text{fresh}\ \alpha_i''
%     \\
%     \Delta \models (\wedge\ \widebar{\alpha'_i \rightarrow \alpha''_i}) \leq \tau
%     \\\\
%     (\text{patenv}\ m_i) \sqsupseteq \Gamma
%     \\
%     \Delta \cdot \Gamma \models x'_i \mapsto m_i : \alpha'_i
%     \\\\
%     \Delta \cdot \Gamma \models x''_i \mapsto e_i : \alpha''_i
%     \\
%     (\wedge\ \widebar{(\Gamma\ x_i') \rightarrow (\Gamma\ x_i'')}) 
%     \sqsupseteq 
%     (\Gamma\ x)
%   } {
%     \Delta\ \cdot \Gamma
%     \models 
%     x \mapsto (\lambda\ \widebar{m_i \Rightarrow e_i}) : \tau
%   } 

%   \inferrule { 
%     \Delta \cdot \Gamma \cdot \models x' \mapsto e : (l : \tau)
%     \\
%     \text{fresh}\ \alpha
%     \\\\
%     \Delta \models (\Gamma\ x') \leq (l : \alpha)
%     \\
%     \alpha \sqsupseteq (\Gamma\ x)
%   } {
%     \Delta\ \cdot \Gamma \models x \mapsto (e/l) : \tau
%   } 

%   \inferrule { 
%     \text{fresh}\ \alpha
%     \\
%     \Delta \cdot \Gamma \cdot \models x_1 \mapsto e_1 : (\alpha \rightarrow \tau)
%     \\
%     \Delta \cdot \Gamma \cdot \models x_2 \mapsto e_2 : \alpha
%     \\\\
%     \text{fresh}\ \alpha'
%     \\
%     \Delta \models (\Gamma\ x_1) \leq (\Gamma\ x_2) \rightarrow \alpha'
%     \\\\
%     \text{fresh}\ \alpha''
%     \\
%     % need to call unification one more time to liberate existentially quantified variables 
...
%     \alpha'' \sqsupseteq (\Gamma\ x)
%   } {
%     \Delta\ \cdot \Gamma \cdot \alpha'' \models x \mapsto (e_1\ e_2) : \tau
%   } 
%   \\\\
%   \inferrule { 
%     \Delta \cdot \Gamma \cdot \models x_1 \mapsto e_1 : \tau_1 
%     \\
%     (\Gamma\ x_1)[\Delta] = \tau_1'
%     \\
%     \text{freevars}\ \tau_1' = \widebar{\alpha}
%     \\\\
%     \Gamma\ x = \forall \widebar{\alpha} . \tau_1'
%     \\
%     \Delta \cdot \Gamma \cdot \models x_2 \mapsto e_2 : \tau_2
%     \\
%     (\Gamma\ x_2) \sqsupseteq (\Gamma\ x)
%   } {
%     \Delta\ \cdot \Gamma\models (\text{let}\ x : \tau_1 = e_1\ \text{in}\ e_2) : \tau_2
%   } 

%   \inferrule { 
%     \Delta \cdot \Gamma \models x' \mapsto e : \bot \rightarrow \tau 
%     \\
%     \text{fresh}\ \alpha_1
%     \\
%     \text{fresh}\ \alpha_2
%     \\\\
%     \Delta \models (\Gamma\ x') \leq \alpha_1 \rightarrow \alpha_2
%     \\
%     \alpha_1[\Delta] = \tau_1
%     \\
%     \alpha_2[\Delta] = \tau_2
%     \\\\
%     (\text{spiral}\ \tau_1\ \tau_2) \sqsupseteq (\Gamma\ x)
%   } {
%     \Delta\ \cdot \Gamma 
%     \models x \mapsto \text{fix}\ e : \tau
%   } 
% \end{mathpar}
% \end{definition}
